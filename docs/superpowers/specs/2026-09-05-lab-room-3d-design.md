...
## 12. 디테일 패스 (2026-09-05, 사용자 피드백 "너무 단순해 보인다 — 월캐비닛·장비·안전모, 문이 허접")

새 모듈 `site/labs/room/decor.js`(월드 좌표, 호버 대상 아님, 자체 팔레트)가 세 그룹을 만들고 `lab-room.js`가 방 껍데기 다음에 추가한다. `props.buildRoomShell`의 문틀(기둥 2 + 보)은 **제거**하고 `buildBayDoor`가 대신한다.

| 그룹 | 내용 |
|---|---|
| `buildBayDoor(DOOR, ROOM)` | 문 양옆 1.1 m 반벽(+갓돌; 높은 벽은 뒤 스테이션을 가리므로 낮게) · 두꺼운 문설주 2 + 강재 가이드 레일 · 상부 롤업 하우징(Royal) + 드럼 · 절반 내려온 슬랫 커튼 5장(하단 1.5 m 이상 열림) · 문턱 판 + 마당 쪽 노랑/검정 해저드 스트라이프 10칸 |
| `buildWallDecor(ROOM)` | 북벽: 월캐비닛 4칸(흙 시험대 위, y 1.7–2.4, 문 홈·손잡이) · 벽시계 · 안전 표지판(흰 판+빨간 테두리) · 후크 레일에 안전모 4개(노랑·흰·주황·Royal, 반구+챙)와 안전조끼 2벌(주황+반사띠 2) · 문 옆 소화기+표지 · 뒷벽 툴체스트(빨강 서랍 캐비닛, 캐스터). 서벽: 페그보드+공구(수평기·망치·줄자·렌치 2), 삽, 화이트보드, 월캐비닛 2칸, 뒷모서리 선반 랙(상자·양동이·통). 바닥: 통로 양쪽 노란 안전선(x ±1.6) |
| `buildYardExtras(YARD)` | 안전 콘 3개(주황+흰 띠+받침) · 외바퀴 손수레 · 팔레트+시멘트 포대 5개 · 모래 더미 · Royal 배럴 2개 |

- 배치 규칙(테스트 `tools/decor.test.mjs`): 벽 장식은 방 안·벽 높이 이하, 스테이션 발자국과 겹치는 부품은 y ≥ 1.6(캐비닛)만 허용, 통로 x 범위의 바닥 비품은 뒷벽(z < −3.9)에만; 마당 장비는 마당 안·측량 발자국 밖; 문 개구부(z −2…2, x ≈ 6)에는 y 0.05–1.5 구간을 막는 부품이 없다(문턱 제외); 메쉬 수 bayDoor ≤ 40 · wallDecor ≤ 130 · yardExtras ≤ 60.
- 해상도: 사용자 허용에 따라 실린더 24–32 세그먼트, 반구 24×14. 예산을 **draw call ≤ 500**으로 올린다(측정값은 검증 보고에 기록).
- 재질 색은 §7 팔레트에 노랑 `0xf2c200`(안전색), 캐비닛 `0xe6ebef`, 골판지 `0xc8a878`, 모래 `0xd8c49a`, 보드 `0xd9dde2`를 추가한다.
