...
## 2. 사실 검증 원칙
- 수치는 ASTM C143/C172·ACI 211.1·EN 206 값만 쓴다. 조사 파일에서 `CAVEAT`(2차 출처)로 표시된 값(정밀도 SD, 30분 슬럼프 손실)은 숫자를 싣지 않고 정성적으로만 쓴다 — "대략"을 붙인 틀린 숫자도 틀린 숫자다(리뷰 지적).
- Part 1(Mix Design)의 시작 슬럼프 표(2–5 / 3–5 in — 감수제 **투입 전** 값, Part 1 본문 명시)와 ACI 211.1의 진동 다짐 기준 표(1–3 / 1–4 in)는 둘 다 혼화제 전 값이다. 차이는 전제가 아니라 보수성이다: ACI 표는 진동 다짐 콘크리트의 보수적 하한, Part 1 표는 통상 시방·펌프 타설에서 쓰는 높은 값. 본문에서 이렇게 밝힌다.
- 사진 크레딧은 CC 조건대로: 저자·라이선스명 + 라이선스 딛(URL) 링크 + 원본(Commons 파일 페이지) 링크 + "리사이즈·재압축" 변경 고지.
- 과목 코드 없음, 영어, 브랜드 토큰만.
